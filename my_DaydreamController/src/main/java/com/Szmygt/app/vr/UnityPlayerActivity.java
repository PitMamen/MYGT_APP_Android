...
import com.Szmygt.app.vr.provider.GameListProvider;
import com.Szmygt.app.vr.provider.VideoListProvider;
import com.Szmygt.app.vr.provider.VideoMainListProvider;
...
public class UnityPlayerActivity extends Activity implements BluetoothLeService.Callback {
	...
	private VideoListProvider mVideoListProvider;
	private VideoMainListProvider mVideoMainListProvider;
	private GameListProvider mGameListProvider;
	private  VideoVo vo;
	...
	private void connectService() {
		if (mBluetoothLeService != null) {
			...
			for (String address : addressList) {
				...
				Log.d("pxk", "boolean_conneted==: " + result + "   ********");
				//写数据
...
//        key3 = (mData[16]>> 4) & 3;
		...
		for (int i = 0; i < 4; i++) {
			...
			mFloatdata[i] = mFloat;   //每个float 都用一个float数组保存起来
...
//        mUnityPlayer = new UnityPlayActivity(this);
//        setContentView(mUnityPlayer);
//        mUnityPlayer.requestFocus();

		if (savedInstanceState == null) {
			mVideoListProvider =new VideoListProvider(this);
			mVideoMainListProvider = new VideoMainListProvider(this);
			mGameListProvider = new GameListProvider(this);
			vo = new VideoVo(this);
			mGameVo = new GameVo(this);


			ObjectStore.put("VideoListProvider",mVideoListProvider);
			ObjectStore.put("VideoMainListProvider",mVideoMainListProvider);
			ObjectStore.put("GameMainListProvider",mGameListProvider);
			ObjectStore.put("UnityPlayerActivity", UnityPlayerActivity.this);
			ObjectStore.put("vo",vo);
			ObjectStore.put("GameInfo",mGameVo);
//			mVideoListProvider.DoScan();
		}
